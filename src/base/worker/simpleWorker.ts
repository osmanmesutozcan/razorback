...
import { PolyfillPromise } from '../winjs.polyfill.promise';
...
export class SimpleWorkerClient<T> extends Disposable {
  ...
  constructor(workerFactory: IWorkerFactory, moduleId: string) {
    ...
    if (typeof (<any>this).require !== 'undefined' && typeof (<any>this).require.getConfig === 'function') {
      // Get the configuration from the Monaco AMD Loader
      loaderConfiguration = (<any>this).require.getConfig();
    } else if (typeof (<any>this).requirejs !== 'undefined') {
      // Get the configuration from requirejs
      loaderConfiguration = (<any>this).requirejs.s.contexts._.config;
    }
...
export class SimpleWorkerServer {
  ...
  private initialize(workerId: number, moduleId: string, loaderConfig: any): Promise<string[]> {
    ...
    if (loaderConfig) {
      ...
      (<any>this).require.config(loaderConfig);
    }

    return new Promise<string[]>((resolve, reject) => {
      // Use the global require to be sure to get the global config
      (<any>this).require([moduleId], (...result: any[]) => {
