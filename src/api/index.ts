...
export function createApiFactory(coreContext: CoreContext) {
  ...
  const extHostCommands = coreContext.constant(
    ExtHostBindings.ExtHostCommands,
    new ExtHostCommands(coreContext)
  );
  const extHostExtensions = coreContext.constant(
    ExtHostBindings.ExtHostExtensions,
    new ExtHostExtensions(coreContext)
  );
  const extHostMessageService = coreContext.constant(
    ExtHostBindings.ExtHostMessageService,
    new ExtHostMessageService(coreContext)
  );
  const extHostWorkspace = coreContext.constant(
    ExtHostBindings.ExtHostWorkspace,
    new ExtHostWorkspace(coreContext)
  );
  const extHostDocuments = coreContext.constant(
    ExtHostBindings.ExtHostDocuments,
    new ExtHostDocuments(coreContext)
  );
  const extHostLanguages = coreContext.constant(
    ExtHostBindings.ExtHostLanguages,
    new ExtHostLanguages(coreContext)
  );
  const extHostConfiguration = coreContext.constant(
    ExtHostBindings.ExtHostConfiguration,
    new ExtHostConfiguration()
  );
  const extHostFileSystemEvents = coreContext.constant(
    ExtHostBindings.ExtHostFileSystemEvents,
    new ExtHostFileSystemEvents(coreContext)
  );
  ...
  return function createApi(
    ...
    const languages: typeof rback.languages = {
      ...
      setLanguageConfiguration: (
        language: string,
        configuration: rback.LanguageConfiguration
      ): rback.Disposable => {
        return extHostLanguages.setLanguageConfiguration(
          language,
          configuration
        );
      },
    ...
    const workspace: typeof rback.workspace = {
      ...
      onDidOpenTextDocument: (listener, thisArgs?, disposables?) => {
        return extHostDocuments.onDidOpenTextDocument(
          listener,
          thisArgs,
          disposables
        );
      ...
      onDidCloseTextDocument: (listener, thisArgs?, disposables?) => {
        return extHostDocuments.onDidCloseTextDocument(
          listener,
          thisArgs,
          disposables
        );
      ...
      onDidSaveTextDocument: (listener, thisArgs?, disposables?) => {
        return extHostDocuments.onDidSaveTextDocument(
          listener,
          thisArgs,
          disposables
        );
      ...
      onDidChangeWorkspaceFolders: (listeners, thisArgs?, disposables?) => {
        return extHostWorkspace.onDidChangeWorkspaceFolders(
          listeners,
          thisArgs,
          disposables
        );
      ...
      createFileSystemWatcher(
        ...
        return extHostFileSystemEvents.createFileSystemWatcher(
          typeConverters.GlobPattern.from(globPattern),
          ignoreCreateEvents,
          ignoreChangeEvents,
          ignoreDeleteEvents
        );
      ...
      getConfiguration(
        section?: string,
        resource?: rback.Uri | null
      ): rback.WorkspaceConfiguration {
        const _resource =
          arguments.length === 1 && resource !== null ? undefined : resource;
        return extHostConfiguration.getConfiguration(
          section,
          _resource!,
