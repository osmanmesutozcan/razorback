...
export class ConfigurationModel implements IConfigurationModel {
  ...
  override(identifier: string): ConfigurationModel {
    ...
    for (const key of _.uniq([
      ...
      if (overrideContentsForKey) {
        // Clone and merge only if base contents and
        // override contents are of type object otherwise just override
        if (typeof contentsForKey === 'object' && typeof overrideContentsForKey === 'object') {
          contentsForKey = _.cloneDeep(contentsForKey);
  ...
  merge(...others: ConfigurationModel[]): ConfigurationModel {
    const contents = _.cloneDeep(this.contents);
    ...
    for (const other of others) {
      ...
      for (const otherOverride of other.overrides) {
        const [override] = overrides
          .filter((o: any) => arrays.equals(o.identifiers, otherOverride.identifiers));
        if (override) {
          this.mergeContents(override.contents, otherOverride.contents);
        } else {
          overrides.push(_.cloneDeep(otherOverride));
        }
      }
  ...
  private mergeContents(source: any, target: any): void {
    for (const key of Object.keys(target)) {
      ...
      source[key] = _.cloneDeep(target[key]);
    }
  }
  ...
  public setValue(key: string, value: any) {
    this.addKey(key);
    addToValueTree(this.contents, key, value, (e: any) => { throw new Error(e); });
  }
      ...
      if (OVERRIDE_PROPERTY_PATTERN.test(key)) {
        overrides.push({
          ...
          contents: toValuesTree(
            contents[key],
            (msg: string) => console.error(`Conflict in default settings file: ${msg}`),
          ),
        });
      }
    }
...
export class Configuration {
  ...
  inspect<C>(key: string, overrides: IConfigurationOverrides, workspace: Workspace | null): {
    ...
    const consolidateConfigurationModel =
      this.getConsolidateConfigurationModel(overrides, workspace);
    const folderConfigurationModel =
      this.getFolderConfigurationModelForResource(overrides.resource, workspace);
    ...
    return {
      default: overrides.overrideIdentifier
        ? this._defaultConfiguration.freeze().override(overrides.overrideIdentifier).getValue(key)
        : this._defaultConfiguration.freeze().getValue(key),

      user: overrides.overrideIdentifier
        ? this._userConfiguration.freeze().override(overrides.overrideIdentifier).getValue(key)
        : this._userConfiguration.freeze().getValue(key),

      workspace: workspace
      ? overrides.overrideIdentifier
        ? this._workspaceConfiguration.freeze().override(overrides.overrideIdentifier).getValue(key)
        : this._workspaceConfiguration.freeze().getValue(key)
      : void 0, // Check on workspace exists or not because _workspaceConfiguration is never null

      workspaceFolder: folderConfigurationModel
      ? overrides.overrideIdentifier
        ? folderConfigurationModel.freeze().override(overrides.overrideIdentifier).getValue(key)
        : folderConfigurationModel.freeze().getValue(key)
      : void 0,

      memory: overrides.overrideIdentifier
        ? memoryConfigurationModel.freeze().override(overrides.overrideIdentifier).getValue(key)
        : memoryConfigurationModel.freeze().getValue(key),
  ...
  private getConsolidateConfigurationModel(
    ...
    const configurationModel =
      this.getConsolidatedConfigurationModelForResource(overrides, workspace);
  ...
  allKeys(workspace: Workspace): string[] {
    const keys = this.keys(workspace);
    const all = [...keys.default];
    const addKeys = (keys: any) => {
...
export class AbstractConfigurationChangeEvent {
  ...
  protected updateKeys(configuration: ConfigurationModel, keys: string[], _resource?: URI): void {
    for (const key of keys) {
...
export class ConfigurationChangeEvent
  ...
  constructor(
    ...
    private _changedConfigurationByResource: ResourceMap<ConfigurationModel> =
    ...
    if (arg1 instanceof ConfigurationChangeEvent) {
      ...
      for (const resource of arg1._changedConfigurationByResource.keys()) {
        ...
        changedConfigurationByResource =
          changedConfigurationByResource.merge(arg1._changedConfigurationByResource.get(resource));
  ...
  get affectedKeys(): string[] {
    ...
    this._changedConfigurationByResource.forEach((model: any) => keys.push(...model.keys));
    return _.uniq(keys);
  }
  ...
  affectsConfiguration(config: string, resource?: URI): boolean {
    const configurationModelsToSearch: ConfigurationModel[] = [this._changedConfiguration];

    if (resource) {
      const model = this._changedConfigurationByResource.get(resource);
      if (model) {
  ...
  private changeWithKeys(keys: string[], resource?: URI): void {
    const changedConfiguration = resource
      ? this.getOrSetChangedConfigurationForResource(resource)
